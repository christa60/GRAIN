"""
Fine-tuning Llama-3.1-8B-Instruct on PubMedQA
==============================================
Uses QLoRA (4-bit quantization + LoRA adapters) via HuggingFace + TRL.
Chat template follows the Llama-3.1-Instruct format:
    ...
    {decision + long_answer}<|eot_id|>

Requirements:
    pip install torch transformers datasets trl peft bitsandbytes accelerate wandb

Recommended hardware: 1x A100/H100 (40GB+) or 2x RTX 3090/4090 for Llama-3.1-8B.

Usage:
    # Basic run
    python finetune_llama_pubmedqa.py

    # With custom args
    python finetune_llama_pubmedqa.py \
        --model_name meta-llama/Meta-Llama-3.1-8B-Instruct \
        --output_dir ./llama31-pubmedqa \
        --num_train_epochs 3 \
        --per_device_train_batch_size 2 \
        --use_wandb True

    # Multi-GPU with accelerate
    accelerate launch finetune_pubmed_llama32.py \
        --model_name meta-llama/Meta-Llama-3.1-8B-Instruct \
        --output_dir ./llama31-pubmedqa/min/42 \
        --num_train_epochs 3 \
...
def format_example(example: dict) -> str:
    """
    Convert a PubMedQA pqa_labeled example into a Llama-3.1-Instruct
    prompt+completion string.

    PubMedQA fields used:
        example["question"]           : str
        example["context"]["contexts"]: list[str]
        example["final_decision"]     : "yes" | "no" | "maybe"
        example["long_answer"]        : str
    """
    ...
    return (
        ...
        f"{_header('user')}{user_content}{EOT}"
        f"{ASSISTANT_HEADER}{assistant_content}{EOT}"
    )
...
class ScriptArgs:
    ...
    output_dir: str = field(default="./llama31-pubmedqa-qlora")
    ...
    wandb_project: str = field(default="llama31-pubmedqa")
...
def build_datasets(args: ScriptArgs):
...
def evaluate_pubmedqa(
    ...
    """
    Run greedy decoding on `num_samples` eval examples and compute
    exact-match accuracy on the final yes/no/maybe decision.
    """
    model.eval()
    tokenizer.padding_side = "left"  # left-pad at inference for correct generation alignment
    ...
    for i in tqdm(range(0, len(samples), batch_size), desc="Evaluating"):
        ...
        gold_answers = [
            extract_decision(ex["text"].split(ASSISTANT_HEADER, 1)[-1])
            for ex in batch
        ]
...
# ---------------------------------------------------------------------------
# Expected formatted sample (Llama-3.1-Instruct template):
# ---------------------------------------------------------------------------
