...
Usage:
    accelerate launch finetune_pubmed_qwen2.py \
        ...
        --output_dir ./qwen2-pubmedqa/acc/42 \
        --num_train_epochs 10 \
...
def format_example(example: dict) -> str:
    """Convert a PubMedQA example into a chat-style prompt+completion string."""
...
class ScriptArgs:
    ...
    output_dir: str = field(default="./qwen2-pubmedqa-qlora")
    ...
    wandb_project: str = field(default="qwen2-pubmedqa")
...
def add_noise_to_weights(model, noise_lambda=0.1):
    """
    Add uniform noise scaled by each parameter's std.
    Noise ~ Uniform(-0.5, 0.5) * noise_lambda * std(param)
    """
    state_dict = model.state_dict()
    
    for name, param in model.named_parameters():
        if param.dtype in [torch.float16, torch.bfloat16, torch.float32]:
            noise = (torch.rand(param.size()) - 0.5) * noise_lambda * torch.std(param)
            state_dict[name][:] += noise.to(param.device)
    
    model.load_state_dict(state_dict)
    print(f"Noise added with lambda={noise_lambda}")
    return model
...
def build_model_and_tokenizer(args: ScriptArgs):
    ...
    # ── Add noise after loading ──────────────────────────────────────────────
    if getattr(args, "noise_lambda", 0.0) > 0:
        model = add_noise_to_weights(model, noise_lambda=args.noise_lambda)
    # ────────────────────────────────────────────────────────────────────────
...
def build_datasets(args: ScriptArgs, tokenizer):
    ...
    # pqa_labeled only ships a train split — carve out 10% for eval
...
def evaluate_pubmedqa(model, tokenizer, dataset, num_samples: int = 100, batch_size: int = 4) -> dict:
    """
    ...
    # Build inference prompts (question + context only, no answer)
    ...
    for i in tqdm(range(0, len(samples), batch_size)):
        ...
        gold_answers = [
            extract_decision(ex["text"].split("<|start_header_id|>assistant<|end_header_id|>")[-1])
            for ex in batch
        ]
        inputs = tokenizer(
            prompts,
...
def main():
    ...
    results = evaluate_pubmedqa(trainer.model, tokenizer, test_ds, num_samples=len(test_ds), batch_size=args.per_device_eval_batch_size)
...
# Expected formatted sample:
# <|begin_of_text|><|start_header_id|>system<|end_header_id|>
# You are a biomedical research assistant. Given a clinical question and related
# research context, respond with one of: 'yes', 'no', or 'maybe', then provide
# a concise explanation grounded in the context.
# Format your response as:
# Answer: <yes|no|maybe>
#
# Explanation: <your reasoning><|eot_id|>
...
# formation, suggesting a causal role...<|eot_id|>
# <|start_header_id|>assistant<|end_header_id|>
# Answer: yes
#
# Explanation: Multiple lines of evidence support mitochondrial dysfunction as
# a contributor to Alzheimer's pathology...<|eot_id|>